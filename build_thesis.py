"""Build the final thesis docx for Aiszr.

Goals:
  - Cover page rebuilt to match 附件4 template visuals (blank lines for the
    student to fill in manually).
  - Abstract + keywords with correct typographic format.
  - TOC re-rendered as plain styled paragraphs (no Word field) so the user
    sees the right look immediately. Page numbers are placeholders that
    Word can refresh later if needed.
  - Body 1..6 with H1 page-break-before so every chapter starts on a new page.
  - Figures inserted with captions, tables for "关键参数表" and "开发问题与解决方案".
  - Arabic page numbers in footer.
  - Expanded 5.5 开发问题与解决方案 prose, plus 2 new entries.

Output: 毕业论文_基于大语言模型的直播互动智能助手系统设计与实现_最终格式版.docx
"""

import os
import copy
from docx import Document
from docx.shared import Pt, Cm, Inches, RGBColor, Mm
...
def _set_cn_font(run, font_name):
    """python-docx只设置 ASCII 字体，中文字体要通过 rPr/rFonts 显式设。"""
...
def _add_run(p, text, font_name=FONT_SONG, size_pt=12, bold=False, italic=False,
...
def _pfmt(p, *, align=None, line_spacing=1.5, space_before_pt=0, space_after_pt=0,
          first_line_indent_chars=None, keep_with_next=False, page_break_before=False):
    ...
    if first_line_indent_chars is not None:
        # 12pt 小四 -> firstLineChars works via XML
        pPr = p._element.get_or_add_pPr()
        ind = pPr.find(qn("w:ind"))
        if ind is None:
            ind = OxmlElement("w:ind")
            pPr.append(ind)
        # 24 = 2 chars (单位 = 1/100 char)
        ind.set(qn("w:firstLineChars"), str(first_line_indent_chars * 100))
        ind.set(qn("w:firstLine"), str(int(first_line_indent_chars * 240)))  # fallback in DXA

def add_body(doc, text, *, indent=True):
    p = doc.add_paragraph()
    _pfmt(p, align=WD_ALIGN_PARAGRAPH.JUSTIFY, line_spacing=1.5,
          first_line_indent_chars=2 if indent else None)
    _add_run(p, text, FONT_SONG, 12)
    return p

def add_heading1(doc, text, *, page_break_before=True):
    p = doc.add_paragraph()
    _pfmt(p, align=WD_ALIGN_PARAGRAPH.CENTER, line_spacing=1.5,
          space_before_pt=24, space_after_pt=12, keep_with_next=True,
          page_break_before=page_break_before)
    _add_run(p, text, FONT_SONG, 15, bold=True)
    return p

def add_heading2(doc, text):
    p = doc.add_paragraph()
    _pfmt(p, align=WD_ALIGN_PARAGRAPH.LEFT, line_spacing=1.5,
          space_before_pt=12, space_after_pt=6, keep_with_next=True)
    _add_run(p, text, FONT_SONG, 12, bold=True)
    return p

def add_centered_line(doc, text, font_name=FONT_SONG, size_pt=12, bold=False,
                      line_spacing=1.5, space_before_pt=0, space_after_pt=0):
    p = doc.add_paragraph()
    _pfmt(p, align=WD_ALIGN_PARAGRAPH.CENTER, line_spacing=line_spacing,
          space_before_pt=space_before_pt, space_after_pt=space_after_pt)
    _add_run(p, text, font_name, size_pt, bold=bold)
    return p

def add_blank(doc, n=1, line_spacing=1.5):
    for _ in range(n):
        p = doc.add_paragraph()
        _pfmt(p, line_spacing=line_spacing)
    return None
...
def add_figure(doc, image_path, caption, width_inch=5.4):
    p = doc.add_paragraph()
    _pfmt(p, align=WD_ALIGN_PARAGRAPH.CENTER, line_spacing=1.0,
          space_before_pt=6, space_after_pt=0, keep_with_next=True)
    run = p.add_run()
    run.add_picture(image_path, width=Inches(width_inch))

    cap = doc.add_paragraph()
    _pfmt(cap, align=WD_ALIGN_PARAGRAPH.CENTER, line_spacing=1.5,
          space_before_pt=0, space_after_pt=6)
    _add_run(cap, caption, FONT_SONG, 10.5)
    return cap
...
def add_table(doc, headers, rows, col_widths_cm=None):
    ...
    def _fill_cell(cell, text, bold=False, size=10.5):
        cell.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
        # Clear default paragraph
        cell.paragraphs[0].text = ""
        para = cell.paragraphs[0]
        _pfmt(para, align=WD_ALIGN_PARAGRAPH.CENTER, line_spacing=1.25,
              space_before_pt=2, space_after_pt=2)
        _add_run(para, text, FONT_SONG, size, bold=bold)

    for ci, h in enumerate(headers):
        _fill_cell(t.rows[0].cells[ci], h, bold=True)
    for ri, row in enumerate(rows, start=1):
        for ci, val in enumerate(row):
            _fill_cell(t.rows[ri].cells[ci], val)
    return t

# Build a section footer with arabic page number ---------------------------

def _add_page_number_to_footer(section, suppress=False):
    """Add a centered Arabic page number to the section's footer."""
    ...
    _set_cn_font(run, FONT_SONG)
    run.font.size = Pt(9)

def _add_header_text(section, text, link_to_prev=False):
    header = section.header
    header.is_linked_to_previous = link_to_prev
...
def _set_section_page_start(section, start_at):
...
def build():
    ...
    # Cover has no page number, no header — keep header empty + footer empty.
    _add_page_number_to_footer(sec0, suppress=True)

    add_blank(doc, 1)
    # School logo centered (from template image1)
    logo_path = os.path.join(FIG_DIR, "school_logo.png")
    if os.path.exists(logo_path):
        p = doc.add_paragraph()
        _pfmt(p, align=WD_ALIGN_PARAGRAPH.CENTER, line_spacing=1.0,
              space_before_pt=0, space_after_pt=18)
        p.add_run().add_picture(logo_path, width=Inches(4.5))
    add_centered_line(doc, "嘉 兴 南 洋 职 业 技 术 学 院", FONT_HEI, 22, bold=False,
                      line_spacing=1.5, space_after_pt=18)
    add_centered_line(doc, "毕 业 设 计（论 文）", FONT_HEI, 30, bold=False,
                      line_spacing=1.5, space_after_pt=36)
    add_blank(doc, 3)

    def cover_field(label):
        """Cover form field: label + visible underline for handwriting later."""
        p = doc.add_paragraph()
        _pfmt(p, align=WD_ALIGN_PARAGRAPH.CENTER, line_spacing=1.75,
              space_before_pt=8, space_after_pt=8)
        _add_run(p, label + "  ", FONT_HEI, 14, bold=False)
        # Visible underscore line (always renders, regardless of underline-on-space)
        _add_run(p, "＿" * 18, FONT_HEI, 14, bold=False)
        return p
    ...
    add_centered_line(doc, "嘉兴南洋职业技术学院“双高”建设办公室印制", FONT_SONG, 16,
                      bold=True)

    # ------------------------------------------------------------------
    # New section for ABSTRACT (page header begins here, footer continues)
    # ------------------------------------------------------------------
    add_page_break(doc)
    # Add section break so we can scope the header to "abstract onward"
    sec_abs = doc.add_section()
    sec_abs.page_height = Mm(297); sec_abs.page_width = Mm(210)
    sec_abs.top_margin = Mm(30); sec_abs.bottom_margin = Mm(25)
    sec_abs.left_margin = Mm(25); sec_abs.right_margin = Mm(25)
    sec_abs.header_distance = Mm(15); sec_abs.footer_distance = Mm(15)
    _add_header_text(sec_abs, "嘉兴南洋职业技术学院毕业设计（论文）", link_to_prev=False)
    _add_page_number_to_footer(sec_abs)
    _set_section_page_start(sec_abs, 1)

    add_centered_line(doc, "基于大语言模型的直播互动智能助手系统设计与实现",
                      FONT_HEI, 18, bold=True, space_before_pt=12, space_after_pt=18)

    # 摘要 paragraph (no first-line indent in original template; we follow user's content)
    p = doc.add_paragraph()
    _pfmt(p, align=WD_ALIGN_PARAGRAPH.JUSTIFY, line_spacing=1.5,
          space_before_pt=6, space_after_pt=6)
    _add_run(p, "【摘要】", FONT_HEI, 12, bold=True)
    _add_run(p,
    ...
    _pfmt(p, align=WD_ALIGN_PARAGRAPH.JUSTIFY, line_spacing=1.5,
          space_before_pt=6, space_after_pt=6)
    _add_run(p, "【关键词】", FONT_HEI, 12, bold=True)
    _add_run(p, "大语言模型；直播互动；弹幕抓取；智能回复；语音克隆；OBS联动",
             FONT_SONG, 12)

    add_page_break(doc)

    # ------------------------------------------------------------------
    # TOC (manual styled rendering; user can refresh by F9 in Word if they
    # later swap to a real field-driven TOC)
    # ------------------------------------------------------------------
    p = doc.add_paragraph()
    _pfmt(p, align=WD_ALIGN_PARAGRAPH.CENTER, line_spacing=1.5,
          space_before_pt=24, space_after_pt=12, keep_with_next=True)
    _add_run(p, "目　　录", FONT_HEI, 16, bold=True)
    ...
    for level, name, page in TOC:
        ...
        if level == 2:
            pPr = p._element.get_or_add_pPr()
            ind = OxmlElement("w:ind"); ind.set(qn("w:left"), "420")
            pPr.append(ind)
        # Right tab stop with dot leader
        p.paragraph_format.tab_stops.add_tab_stop(
            Mm(160 - (10 if level == 2 else 0)),
            alignment=WD_TAB_ALIGNMENT.RIGHT, leader=WD_TAB_LEADER.DOTS)
        _add_run(p, name, FONT_SONG, 12)
        _add_run(p, "\t" + str(page), FONT_SONG, 12)

    add_page_break(doc)

    # ------------------------------------------------------------------
    # New section for body — Arabic page numbering, restart at 1
    # ------------------------------------------------------------------
    sec_body = doc.add_section()
    sec_body.page_height = Mm(297); sec_body.page_width = Mm(210)
    sec_body.top_margin = Mm(30); sec_body.bottom_margin = Mm(25)
    sec_body.left_margin = Mm(25); sec_body.right_margin = Mm(25)
    sec_body.header_distance = Mm(15); sec_body.footer_distance = Mm(15)
    _add_header_text(sec_body, "嘉兴南洋职业技术学院毕业设计（论文）", link_to_prev=False)
    _add_page_number_to_footer(sec_body)
    _set_section_page_start(sec_body, 1)

    # ------------------------------------------------------------------
    # CHAPTER 1 — 绪论
    # ------------------------------------------------------------------
    add_heading1(doc, "1 绪论", page_break_before=False)

    add_heading2(doc, "1.1 课题的研究背景")
    add_body(doc,
    ...
    add_body(doc,
        ...
        "导致互动体验下降，进而影响销售转化率。")
    add_body(doc,
        "为了解决上述问题，业界开始探索利用人工智能技术辅助直播运营。基于大语言模型（LLM）"
        "的智能回复系统能够自动理解观众提问并生成高质量回复，语音合成技术可以将文字回复"
        "转化为语音播报，OBS联动技术可以实现关键词触发的场景自动切换。"
    ...
    # CHAPTER 2 — 相关技术概述
    # ------------------------------------------------------------------
    add_heading1(doc, "2 相关技术概述")
    add_heading2(doc, "2.1 Playwright 浏览器自动化技术")
    add_body(doc,
    ...
    add_heading2(doc, "2.2 Protobuf 协议与 WebSocket 通信")
    add_body(doc,
    ...
    add_heading2(doc, "2.3 大语言模型与 DeepSeek API")
    add_body(doc,
    ...
    add_heading2(doc, "2.4 阿里云百炼语音克隆技术")
    add_body(doc,
    ...
    add_heading2(doc, "4.3 AI 智能回复引擎设计")
    add_body(doc,
        "AI智能回复引擎是本系统的核心创新点，"
    ...
    add_heading2(doc, "4.5 模块依赖与关键参数")
    add_body(doc,
    ...
    add_heading2(doc, "5.1 登录会话管理与弹幕抓取")
    add_body(doc,
    ...
    add_heading2(doc, "5.2 协议解码与消息处理")
    add_body(doc,
    ...
    add_heading2(doc, "5.3 AI 智能回复引擎实现")
    add_body(doc,
    ...
    add_heading2(doc, "5.4 OBS 联动与数字人推流")
    add_body(doc,
    ...
    add_heading2(doc, "5.5 开发问题与解决方案")
    add_body(doc,
    ...
    refs = [
        "[1] 深度求索. DeepSeek API 文档[EB/OL]. "
        "https://platform.deepseek.com/api-docs, 2025.",
        "[2] 微软. Playwright Python 文档[EB/OL]. "
        "https://playwright.dev/python/, 2025.",
        "[3] 阿里云. DashScope 语音合成 API 文档[EB/OL]. "
        "https://help.aliyun.com/document_detail/2712536.html, 2025.",
        "[4] OBS Project. OBS WebSocket v5 Protocol[EB/OL]. "
        "https://github.com/obsproject/obs-websocket/blob/master/docs/generated/protocol.md, 2024.",
        "[5] Google. Protocol Buffers Documentation[EB/OL]. "
        "https://protobuf.dev/, 2025.",
        ...
        "[7] zhonghangAlex. DySpider: 抖音直播弹幕抓取[EB/OL]. "
        "https://github.com/zhonghangAlex/DySpider, 2024.",
        "[8] OpenAI. GPT-4 Technical Report[J]. arXiv preprint arXiv:2303.08774, 2023.",
        "[9] Touvron H, Lavril T, Izacard G, et al. LLaMA: Open and Efficient "
        "Foundation Language Models[J]. arXiv preprint arXiv:2302.13971, 2023.",
        "[10] FastAPI. FastAPI Framework Documentation[EB/OL]. "
